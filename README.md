...
Примечание: символы `{}` по краям регулярного выражения используются для его выделения
...
2. Одно- двухбуквенные союзы и предлоги внутри текста хорошо бы не отрывать от многобуквенных слов (какие-то прилепляя к следующему слову, какие-то — к предыдущему):
 ...
 1. Поиск где автор использует личные местоимения:`{[ ˚~\\,.!?,«](я|мне|меня|мы|мной|мною|нас|нам|нами|нас|моей|моему|мой|моего)[ ˚~\\,.!?,»…]}`
 
 # Директивы компилятора pdflatex для сборки книги
 -synctex=-1 
 -shell-escape